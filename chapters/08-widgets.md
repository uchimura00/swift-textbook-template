...
**何をしているか：**
TimelineProviderはウィジェットに渡すデータのタイミングと内容を決めるもの。
placeholder()はウィジェットがまだ値を取りに行く前に、ダミー表示をするために。
getSnapshot()はウィジェットギャラリー内のPreview表示時に実データを使ってPreviewを返すもの。
getTimeline()は1つ以上のTimelineEntryと「次の更新方針（TimelineReloadPolicy）」を返し、システムがそのスケジュールに従って表示・更新できるようにするため。コードでは「今日の名言」を表示し、次は「翌日の0時に更新」するよう policy: .after(tomorrow) を返している。

**なぜこう書くのか：**
それぞれ三つとも用途が異なるため三つに分けている。それぞれの用途に適した内容を返すことで表示の速さや省電力化を実現している。

**もしこう書かなかったら：**
ios17以降に登場したAppIntentTileProviderを使う選択肢もある。基本a的な実装は同じだが、asyncで書けてIntentも受け取れるようになる。
