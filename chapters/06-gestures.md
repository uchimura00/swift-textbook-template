...
**もしこう書かなかったら：**
固定の座標やカードの位置だけで判定すると、どこからドラッグしたのかの判定が難しくなる。また、しきい値を使わずに少し左右に動かしただけでスワイプ判定すると、ユーザーの想定通りの挙動にならない。

---

## この章のまとめ
gestureを使ったジェスチャー操作の実装方法やgesture操作をアニメーションとうまく連携させる応用例から、ジェスチャー操作の実装について基本的なことを学んだ。
